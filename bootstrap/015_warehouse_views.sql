...
create or replace view reporting.warehouse_sessions as
    select * exclude (period_plus, record_type, session_type) from internal_reporting_mv.CLUSTER_AND_WAREHOUSE_SESSIONS_COMPLETE_AND_DAILY where RECORD_TYPE = IN('DAILY','COMPLETE2') AND session_type = 'WH'
    union all
    select * exclude (period_plus, record_type, session_type) from internal_reporting_mv.CLUSTER_AND_WAREHOUSE_SESSIONS_COMPLETE_AND_DAILY_INCOMPLETE where RECORD_TYPE = IN('DAILY','COMPLETE2') AND session_type = 'WH'
    ;


create or replace view reporting.cluster_sessions as
    select * exclude (period_plus, record_type, session_type) from internal_reporting_mv.CLUSTER_AND_WAREHOUSE_SESSIONS_COMPLETE_AND_DAILY where RECORD_TYPE IN('DAILY','COMPLETE2') AND session_type = 'CL'
    union all
    select * exclude (period_plus, record_type, session_type) from internal_reporting_mv.CLUSTER_AND_WAREHOUSE_SESSIONS_COMPLETE_AND_DAILY_INCOMPLETE where RECORD_TYPE = ('DAILY','COMPLETE2') AND session_type = 'CL'
    ;

create or replace view reporting.warehouse_sessions_daily as
    select * exclude (period_plus, record_type, session_type) from internal_reporting_mv.CLUSTER_AND_WAREHOUSE_SESSIONS_COMPLETE_AND_DAILY where RECORD_TYPE IN ('COMPLETE','DAILY2')
    union all
    select * exclude (period_plus, record_type, session_type) from internal_reporting_mv.CLUSTER_AND_WAREHOUSE_SESSIONS_COMPLETE_AND_DAILY_INCOMPLETE where RECORD_TYPE = IN ('COMPLETE','DAILY2');

create or replace view reporting.cluster_sessions_daily as
    select * exclude (period_plus, record_type, session_type) from internal_reporting_mv.CLUSTER_AND_WAREHOUSE_SESSIONS_COMPLETE_AND_DAILY where RECORD_TYPE = ('COMPLETE','DAILY2') AND session_type = 'CL'
    union all
    select * exclude (period_plus, record_type, session_type) from internal_reporting_mv.CLUSTER_AND_WAREHOUSE_SESSIONS_COMPLETE_AND_DAILY_INCOMPLETE where RECORD_TYPE = ('COMPLETE','DAILY2') AND session_type = 'CL';
